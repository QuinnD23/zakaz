    ...
    # *****
        ...
        if check_num:
            ...
            if check_table:
                ...
                await StateMachine.EditWorkersServicesCommands.set()
            # check table
            else:
    ...
    if message.text == "Добавить Услугу📙":
        ...
        while service_num <= services_count:
            ...
            for i in range(0, len(now_services_array)):
                if service_num == int(now_services_array[i]):
                    check_table = True
                    break
            if not check_table:
        ...
        if message.text == "Стоп⛔️":
            ...
            while True:
                try:
                    ...
                    break
                if services_array_element not in services_array:
                    services_array.append(services_array_element)
                services_position += 1
            services_array = sorted(services_array)
            services = ""
            for i in range(0, len(services_array)):
                services = services + services_array[i] + " "
            services = services[:-1]
            await update_db("workers", "worker_name", "services", add_worker_name, services)

            if user_name == main_admin_name:
                await message.answer("📙Услуги успешно отредактированы", reply_markup=MainAdminMenu)
                await StateMachine.MainAdmin.set()
            else:
                await message.answer("📙Услуги успешно отредактированы", reply_markup=AdminMenu)
