...
async def mess(message: Message):
    ...
    if message.text == "Удалить❌":
        ...
        while counter_members < members_count:
            id_member = str(delete_id) + '#' + str(counter_members)
            try:
                member_name = str(await select_db("notifiesmembers", "id_member", "member_name", id_member))
            except:
                counter_members += 1
        ...
        if message.text == "Стоп⛔️":
            ...
            if check:
                ...
                if check:
                    ...
                    id_member = str(id_notify-1) + '#' + str(await select_db("notifies", "delete_id", "members_count", id_notify))
...
async def mess(message: Message):
    ...
    # -----
        ...
        if message.text == "Стоп⛔️":
            ...
            if check:
                delete_id_member = str(delete_id) + '#' + str(delete_id_member)
                check = True
                try:
